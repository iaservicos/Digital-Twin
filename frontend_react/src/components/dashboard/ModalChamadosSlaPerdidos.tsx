...
export const ModalChamadosSlaPerdidos: React.FC<ModalChamadosSlaPerdidosProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8 bg-slate-950/60 dark:bg-slate-950/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-slate-800 dark:text-slate-100 relative transition-colors">
        
        {/* Top Header */}
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-50/80 dark:bg-slate-950/60">
          <div>
            ...
            <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">Chamados encerrados fora do SLA</h2>
          </div>

          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/50 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer self-end sm:self-center"
            title="Fechar Modal"
          >
            <X size={20} />
          </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 sm:p-6 bg-slate-50/50 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800/80">
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 flex flex-col shadow-xs">
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Atingimento SLA</span>
            <div className="flex items-baseline gap-1.5 mt-1">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">/ Meta ≥ 90%</span>
            </div>
          </div>

          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 flex flex-col shadow-xs">
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Chamados Fora SLA</span>
            <div className="flex items-baseline gap-1.5 mt-1">
              <span className="text-2xl font-black text-slate-900 dark:text-white">
                {chamados.length}
              </span>
              <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">chamados</span>
            </div>
          </div>

          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 flex flex-col shadow-xs">
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Pontuação do KPI</span>
            <div className="flex items-baseline gap-1.5 mt-1">
              <span className="text-2xl font-black text-slate-900 dark:text-white">
                {pontosSla.toFixed(1)}
              </span>
              <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">/ 32.5 pts</span>
            </div>
          </div>

          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 flex flex-col justify-center shadow-xs">
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Status da Base</span>
            {percentualSla >= 90 ? (
        ...
        <div className="p-4 sm:p-6 pb-2 border-b border-slate-200 dark:border-slate-800/80 flex flex-col gap-3 bg-slate-50/70 dark:bg-slate-900/80">
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-900 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 transition-all shadow-xs"
              />
              {searchTerm && (
                <button
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-hide text-xs">
            <span className="text-slate-500 dark:text-slate-500 font-bold uppercase text-[11px] flex items-center gap-1 whitespace-nowrap mr-1">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Nenhum Chamado Perdido Encontrado</h3>
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/80 pb-3">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-mono font-bold text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-800">
                        #{item.chamado}
                      </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-xs py-1">
                    <div className="bg-slate-50/70 dark:bg-slate-900/60 p-2.5 rounded-lg border border-slate-200/80 dark:border-slate-800/60">
                      <span className="text-slate-500 dark:text-slate-500 font-medium block text-[10px] uppercase">Técnico</span>
                    ...
                    <div className="bg-slate-50/70 dark:bg-slate-900/60 p-2.5 rounded-lg border border-slate-200/80 dark:border-slate-800/60">
                      <span className="text-slate-500 dark:text-slate-500 font-medium block text-[10px] uppercase">Equipamento / Modelo</span>
                      <span className="text-slate-800 dark:text-slate-200 font-semibold flex items-center gap-1 mt-0.5">
                        <Wrench size={13} className="text-slate-400 dark:text-slate-400 shrink-0" />
                    ...
                    <div className="bg-slate-50/70 dark:bg-slate-900/60 p-2.5 rounded-lg border border-slate-200/80 dark:border-slate-800/60">
                      <span className="text-slate-500 dark:text-slate-500 font-medium block text-[10px] uppercase">Base ATP</span>
                      <span className="text-slate-800 dark:text-slate-200 font-semibold flex items-center gap-1 mt-0.5">
                        <Building2 size={13} className="text-slate-400 dark:text-slate-400 shrink-0" />
                  ...
                  {item.textoEncerramento && (
                    <div className="mt-1 bg-slate-50/70 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg p-3">
                      ...
                      <p className="text-xs text-slate-700 dark:text-slate-300 whitespace-pre-line leading-relaxed font-mono bg-slate-100/80 dark:bg-slate-950/90 p-2.5 rounded border border-slate-200 dark:border-slate-800 max-h-36 overflow-y-auto select-text">
                        {item.textoEncerramento}
                      </p>
                    </div>
                  )}
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-600 dark:text-slate-500 bg-slate-50 dark:bg-slate-950">
          <span>Total exibido: <strong className="text-slate-900 dark:text-slate-300">{filteredChamados.length}</strong> de <strong className="text-slate-900 dark:text-slate-300">{chamados.length}</strong> chamados fora do SLA</span>
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 rounded-xl font-bold transition-colors cursor-pointer"
          >
            Fechar
          </button>
        </div>
